...
const isNunjucksFunctionsObject = (x: unknown): x is Record<string, { (...args: unknown[]): unknown }> => (
...
const tryImportModuleCli = async (optionName: string, optionValue: unknown): Promise<unknown> => {
	if (typeof optionValue === 'string') {
		...
		if (typeof module === 'undefined')
			throw new Error(`nunjucks-writer failed to load module specified in '${optionName}' option: imported value is 'undefined'.`);

		return module;
	} else if (isObject(optionValue)) {
		if (typeof optionValue.module !== 'string')
			throw new Error(`nunjucks-writer '${optionName}.module' option is invalid type, expected string.`);
		if (typeof optionValue.export !== 'undefined' && typeof optionValue.export !== 'string')
			throw new Error(`nunjucks-writer '${optionName}.export' option is invalid type, expected string.`);
		...
		throw new Error(`nunjucks-writer '${optionName}' option is invalid type, expected object or string.`);
	}
};

export const nunjucksWriterOptionsFromCliParameters = async (cliParams: Record<string, unknown> = {}) => {
	const {
		view,
		globals,
		functions,
	...
	const options = { ...rest } as NunjucksWriterOptions;
	...
	if (typeof importedFunctions !== 'undefined') {
		if (!isNunjucksFunctionsObject(importedFunctions)) {
			throw new Error('nunjucks-writer failed to load module specified in \'functions\' option: imported value is not a function map.');
		}
	...
	if (typeof importedFilters !== 'undefined') {
		if (!isNunjucksFunctionsObject(importedFilters)) {
			throw new Error('nunjucks-writer failed to load module specified in \'filters\' option: imported value is not a function map.');
		}
	...
	if (typeof importedAdvanced !== 'undefined') {
		if (!isUnknownFunction(importedAdvanced)) {
			throw new Error('nunjucks-writer failed to load module specified in \'advanced\' option: imported value is not a function.');
		}
...
export const cli = async (cliParams: Record<string, unknown> = {}) => nunjucksWriter(await nunjucksWriterOptionsFromCliParameters(cliParams));
